				...
				<div class="our-team-box">
					<div class="container clearfix">
								<?php	$event_our_speaker_get_featured_posts 		= new WP_Query(array(
									'posts_per_page'      	=> absint($event_settings['event_total_our_speaker']),
									'post_type'           	=> array('page'),
									'post__in'            	=> $event_our_speaker_list_page,
									'orderby'             	=> 'post__in',
								));
					if($event_settings['event_our_speaker_title'] != ''){ ?>
						<h2 class="box-title"><?php echo esc_html($event_settings['event_our_speaker_title']);?> </h2>
					<?php } 
					if($event_settings['event_our_speaker_description'] != ''){ ?>
						<p class="box-sub-title"><?php echo esc_html($event_settings['event_our_speaker_description']);?> </p>
					<?php } ?>
						...
						while ($event_our_speaker_get_featured_posts->have_posts()):$event_our_speaker_get_featured_posts->the_post();
							...
							<div class="four-column">
								...
								if($event_settings['event_our_speaker_position_'. $i .''] != ''){ ?>
									<p class="member-post"><?php echo esc_html($event_settings['event_our_speaker_position_'. $i .'']);?> </p>
								<?php } 
								if($event_settings['event_our_speaker_about_'. $i .''] != '') { ?>
									<div class="speaker-topic-title">
										<h4><?php echo esc_html($event_settings['event_our_speaker_about_'. $i .'']);?></h4>
									</div>
								<?php } ?>
